...
// English sitemap — served at https://type10.com/sitemap.xml.
// Lists type10.com URLs with reciprocal hreflang alternates to type10.de.
export const GET: APIRoute = async () => {
  const entries = await getSitemapEntries();
  const body = entries
    .map(({ key, sub }) => {
      const loc = publicUrl(key, 'en', sub);
